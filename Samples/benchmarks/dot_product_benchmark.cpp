...
static void BM_DotProduct_AVX2_One_Accumulator(benchmark::State& state)
{
    const std::size_t n = state.range(0);
    ...
    for (auto _ : state) {
        float result = dot_product_avx2_fma_one_accumulator(a.data(), b.data(), n);
...
static void BM_DotProduct_AVX2_Four_Accumulator(benchmark::State& state)
{
    const std::size_t n = state.range(0);
    ...
    for (auto _ : state) {
        float result = dot_product_avx2_fma_four_accumulators(a.data(), b.data(), n);
...
BENCHMARK(BM_DotProduct_AVX2_One_Accumulator)
    ->Arg(128)
    ->Arg(1024)
    ->Arg(4096)
    ->Arg(16384)
...
BENCHMARK(BM_DotProduct_AVX2_Four_Accumulator)
    ->Arg(128)
    ->Arg(1024)
    ->Arg(4096)
    ->Arg(16384)
